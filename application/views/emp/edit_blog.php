...
<form name="blog_edit" id="blog_edit" method="post" action="emp-update_blog" enctype="multipart/form-data">
<?php
foreach ($b_list as $b)
{
?>
  <input type="hidden" name="b_id" value="<?php echo $b->id; ?>">
    ...
    <div class="form-group col-md-12">
      <label>Title:</label>
      <input type="text" class="form-control" id="title" name="title" value="<?php echo $b->title; ?>" placeholder="Title" required="" readonly>
    </div>
  </div>
  <div class="form-row">
    <div class="form-group col-md-12">
      <label>Writing Title:</label>
      <input type="text" class="form-control" id="wtitle" name="wtitle" value="<?php echo $b->wtitle; ?>" placeholder="Writing Title" required="">
    </div>
  </div>
  <div class="form-row">
    <div class="form-group col-md-12">
      <label>URL:</label>
      <input type="text" class="form-control" id="url" name="url" value="<?php echo $b->url; ?>" placeholder="URL" required="" readonly>
    </div>
  </div>
  <div class="form-row">
    <div class="form-group col-md-12">
  ...
  <div class="form-row">
    <div class="form-group col-md-12">
      <label>Menu:</label>
      <select class="form-control" name="menu" id="menu" required="" readonly>
        <?php
        $id = $b->menu_id;
                    $menu = $this->emp_model->getMenuIDwise($id);
                      foreach ($menu as $r2) 
                      {
                         ?>
                         <option value="<?php echo $b->menu_id; ?>"><?php echo $r2->menu_name; ?></option>
                         <?php
                      }
                ?>
         </select> 
  ...
  <div class="form-row">
    <div class="form-group col-md-12">
      ...
      <select class="form-control" name="sub_menu_id" id="sub_menu_id" readonly>
         <?php
         $id = $b->sub_menu_id;
                    $smenu = $this->emp_model->getSubMenuIDwise($id);
                      ...
                      {
                         ?>
                         <option value="<?php echo $b->sub_menu_id; ?>"><?php echo $sr2->sub_menu_name; ?></option>
                         <?php
                      }
          ?>
         </select> 
